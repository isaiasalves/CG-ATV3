...
void Wall::paintGL() {
  ...
  position = glm::translate(position, glm::vec3(-1.0f, 0.0f, 0.0f));
  position = glm::rotate(position, glm::radians(90.0f), glm::vec3(0, 1, 0));
...
void Wall::loadModelFromFile(std::string_view path, float offset, float scale) {
  m_yoffset = offset;
  m_scale = scale;
...
void Wall::loadDiffuseTexture(std::string_view path) {
...
void Wall::loadNormalTexture(std::string_view path) {
  if (!std::filesystem::exists(path)) return;
